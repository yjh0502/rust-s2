...
mod tests {
    ...
    fn test_siti_st_roundtrip() {
        ...
        for _ in 0..1000 {
            let si = rng.random_range(0..MAX_SITI);
            assert_eq!(st_to_siti(siti_to_st(si)), si);
        }

        for _ in 0..1000 {
            let st = rng.random_range(0.0..1f64);
    ...
    fn test_xyz_to_face_siti() {
        ...
        for level in 0..MAX_LEVEL {
            ...
            {
                let face_random = rng.random_range(0..NUM_FACES);
                let mask = (u64::MAX << (MAX_LEVEL - level)) & (MAX_SITI - 1);
                let si_random = rng.random_range(0..MAX_SITI) & mask;
                let ti_random = rng.random_range(0..MAX_SITI) & mask;
