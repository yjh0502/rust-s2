...
where
    R: Rng,
{
    let base = rng.random_range(0..max_log + 1);
    rng.random_range(0..1 << 31) & ((1 << base) - 1)
}
...
where
    R: Rng,
{
    let cap_area = max_area * (min_area / max_area).powf(rng.random_range(0.0..1.0));
...
where
    R: Rng,
{
    let level = rng.random_range(0..(MAX_LEVEL + 1));
...
where
    R: Rng,
{
    let face = rng.random_range(0..NUM_FACES as u64);
    ...
    R: Rng,
{
    ...
    let h = rng.random_range(0.0..1.0) * c.height();
    let theta = 2. * PI * rng.random_range(0.0..1.0);
    let r = (h * (2. - h)).sqrt();
